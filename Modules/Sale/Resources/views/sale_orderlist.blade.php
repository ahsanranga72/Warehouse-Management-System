<tr class="orderData dataRow{{$product->id}}" data-id="{{$product->id}}">
    ...
    <td class="d-flex">
        ...
        <select name="select-sale-unit{{$product->id}}" id="select-sale-unit{{$product->id}}" class="form-control w-50 select-sale-unit">
            ...
            <option data-unit-id="{{ $key->id }}" value="{{ $key->value }}" {{ ($product->sale_unit==$key->id)?"selected":'' }}>{{ $key->name }}</option>
            @endforeach
    </td>
    <td class="rcvrow"><input type="number" min="0" class="form-control received " name="received{{$product->id}}" id="received{{$product->id}}"></td>
    <td class="unitcost" data-unitcost='{{$product->product_price}}'><input value="{{$sale_price}}" type="number" class="form-control net-unit-cost" required min="0" name="net-unit-cost{{$product->id}}" id="net-unit-cost{{$product->id}}"></td>
